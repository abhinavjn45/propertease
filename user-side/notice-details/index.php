<?php 
    ...
    if (!isset($_GET['notice_id']) && !is_numeric($_GET['notice_id'])) {
        ...
        $notice_details = fetch_single_notice($notice_id);
...
<head>
    <meta charset="UTF-8">
    <title><?= htmlspecialchars($notice_details['notice_title']) ?></title>
...
<body>
        ...
        <section class="hero-section" aria-labelledby="page-title">
            <div class="hero-inner">
                <div class="hero-breadcrumb">
                    <a href="<?= get_site_option('site_url') ?>">Home</a> / <a href="<?= get_site_option('site_url') . 'notices/' ?>">Notices &amp; Circulars</a> / <?= htmlspecialchars(substr($notice_details['notice_title'], 0, 40)) . "..." ?>
                </div>
                <h1 class="hero-title" id="page-title"><?= htmlspecialchars($notice_details['notice_title']) ?></h1>
                <p class="hero-subtitle" id="hero-subheading"><?= htmlspecialchars($notice_details['notice_single_line']) ?></p>
                <a href="<?= get_site_option('site_url') . 'notices/' ?>" class="back-link">
                    <span>←</span>
            ...
            <div class="main-inner">
                ...
                <article class="announcement-detail-card">
                    ...
                    <div class="announcement-header">
                        <div class="announcement-meta-row">
                            <span class="announcement-category" id="category"><?= htmlspecialchars($notice_details['notice_category_name']) ?></span>
                            <?php 
                                if ($notice_details['notice_badge'] !== null && !empty($notice_details['notice_badge'])) {
                                    echo "
                                        <span class='announcement-priority'>
                                            <span class='priority-icon'><i class='fas fa-circle'></i></span>
                                            <span>
                                                " . htmlspecialchars($notice_details['notice_badge']) . "
                                            </span>
                                        </span>
                                    ";
                                }
                        ...
                        <div class="announcement-meta-info">
                            <div class="meta-item">
                                ...
                                <div>
                                    ...
                                    <span id="announcement-date"><?= date("d F, Y", strtotime($notice_details['notice_posted_on'])) ?></span>
                                </div>
                            </div>
                            <div class="meta-item">
                    ...
                    <div class="announcement-content" id="announcement-content">
                        <?= $notice_details['notice_content'] ?>
                    </div>
                    <!-- Action Section -->
                    <?php 
                        $has_main_material = !empty($notice_details['notice_material']);
                        $has_additional = false;
                        if (isset($con)) {
                            $stmtAtt = mysqli_prepare($con, "SELECT 1 FROM additional_attachments WHERE add_attachment_notice_id = ? LIMIT 1");
                            if ($stmtAtt) {
                                mysqli_stmt_bind_param($stmtAtt, 'i', $notice_id);
                                mysqli_stmt_execute($stmtAtt);
                                $resAtt = mysqli_stmt_get_result($stmtAtt);
                                $has_additional = ($resAtt && mysqli_num_rows($resAtt) > 0);
                                mysqli_stmt_close($stmtAtt);
                            }
                        }
                        if ($has_main_material || $has_additional):
                    ?>
                    <div class="action-section">
                        ...
                        <div class="action-buttons">
                            <?php 
                                if ($has_main_material) {
                                    echo "
                                        <a class='btn-action' href='" . get_site_option('dashboard_url') . "assets/uploads/documents/notices/" . htmlspecialchars($notice_details['notice_material']) . "' download>
                                            <span><i class='fas fa-file-pdf'></i></span>
                                            <span>" . htmlspecialchars($notice_details['notice_material_title']) . "</span>
                                        </a>
                                    ";
                                }

                                fetch_additional_attachments('notice', $notice_id);
                            ?>
                        </div>
                    </div>
                    <?php endif; ?>
                ...
                <div class="related-section">
                    ...
                    <ul class="related-list">
                        <?= fetch_related_notices($notice_id) ?>
                    </ul>
                </div>
            </div>
        </main>
    ...
    <!-- Page-specific JS -->
    <script src="../assets/js/single-announcement.js"></script>
